...
def _load_tokens():
    ...
    # 可选：隔离的共享 token 文件（多 agent 场景，宿主把 token 写入受权限保护的文件，
    # 各 agent 进程读取同一份；仅在 env 未注入时生效，env 始终优先）。
    # 安全约定：该文件必须 0600 且仅宿主可读，禁止提交到仓库 / 共享目录。
    tkf = os.environ.get("TDOC_SHARED_TOKEN_FILE", "")
    if tkf and os.path.isfile(tkf):
        try:
            with open(tkf, "r", encoding="utf-8") as fh:
                tk = json.loads(fh.read())
            oauth = oauth or str(tk.get("oauth") or "")
            oneid = oneid or str(tk.get("oneid") or "")
            _debug(f"loaded shared token file: {tkf}")
            return oauth, oneid
        except Exception as e:  # noqa: BLE001
            _debug(f"read TDOC_SHARED_TOKEN_FILE failed: {e}")
...
# ── 主入口 A2：导出共享 token 文件（非 WorkBuddy host 桥接） ──────────────────
def cmd_export_token(args):
    """从当前可用票源取 live token，写入 0600 的 JSON 文件供其他 agent 读取。

    用法： python3 tencentdocs.py tdoc_export_token <path>
    生成的文件内容： {"oauth": "...", "oneid": "..."}
    其他 agent 进程用前设置环境变量： TDOC_SHARED_TOKEN_FILE=<path>
    """
    oauth, oneid = _load_tokens()
    if not oauth and not oneid:
        print("ERROR:no_token - 当前环境无任何可用票源，无法导出；"
              "请确认本 WorkBuddy 会话已授权并连接「腾讯文档」连接器")
        return 1
    path = args.path
    try:
        ...
        with open(path, "w", encoding="utf-8") as fh:
            json.dump({"oauth": oauth, "oneid": oneid}, fh, ensure_ascii=False)
        # 0600：仅属主可读写（Windows 下尽力而为，NTFS 不保证完全等价）
        try:
            os.chmod(path, 0o600)
        except OSError:
            pass
    except OSError as e:
        print(f"ERROR:write_failed - 写 token 文件失败: {e}")
        return 1
    print(f"OK: token 已导出到 {path}（权限 0600）")
    print("提示：其他 agent 调用前设置 TDOC_SHARED_TOKEN_FILE=" + path)
    print("      该文件含实时凭据，切勿进入版本控制 / 共享包；过期后重跑本命令即可")
    return 0
...
def main(argv=None):
    ...
    p = sub.add_parser("tdoc_export_token",
                       help="导出共享 token 文件（TDOC_SHARED_TOKEN_FILE），供非 WorkBuddy 的 agent 读取")
    p.add_argument("path", help="输出路径，如 /path/to/tdoc_token.json（将写为 0600）")
